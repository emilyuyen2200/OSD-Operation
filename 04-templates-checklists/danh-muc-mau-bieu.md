# Danh mục Mẫu biểu & Checklist

**Tài liệu này đang được xây dựng.** Danh sách dưới đây liệt kê các mẫu biểu công ty cần có, nhưng nội dung form thật chưa được soạn. Không dùng danh sách này thay cho biểu mẫu chính thức đang lưu hành — hỏi Exam Coordinator nếu cần bản mẫu hiện tại.

## Nhóm 1 — Quy chế & Phân quyền

| Mẫu biểu | Dùng ở đâu | Trạng thái |
| --- | --- | --- |
| Biên bản (loại chung) | Nhiều bước | Cần soạn |
| Phiếu bàn giao / niêm phong | Bước 4, 7, 9 | Cần soạn |
| Checklist theo vai trò | Tất cả vai trò | Cần soạn |

## Nhóm 2 — Chuẩn bị kỳ thi

| Mẫu biểu | Dùng ở đâu | Trạng thái |
| --- | --- | --- |
| Sơ đồ phòng thi | Bước 5 | Cần soạn |
| Giấy nháp thi | Bước 6 | Cần soạn |
| Kế hoạch kỳ thi | Bước 1 | Cần soạn |
| Tờ phân loại đề thi | Bước 6 | Cần soạn |
| Danh sách check-in | Bước 7 | Cần soạn |
| Danh sách giữ đồ | Bước 7 | Cần soạn |
| Danh sách thi các kỹ năng | Bước 3 | Cần soạn |
| Protokol | Bước 7, 9 | Cần soạn |
| Checklist Phân công Nhân sự | Bước 5 | [Xem](./checklist-phan-cong-nhan-su.md)|
| Checklist Hồ sơ & Biểu mẫu Phòng thi | Bước 5 | [Xem](./checklist-ho-so-bieu-mau-phong-thi.md)|
| Checklist Thiết bị | Bước 5 | [Xem](./checklist-thiet-bi.md) |
| Checklist Setup Phòng thi | Bước 5 | [Xem](./checklist-setup-phong-thi.md) |
| Checklist Phần mềm & Hệ thống | Bước 5 | [Xem](./checklist-phan-mem-va-he-thong.md) |

## Nhóm 3 — Ngày thi & Bàn giao

| Mẫu biểu | Dùng ở đâu | Trạng thái |
| --- | --- | --- |
| Biên bản bàn giao bài thi + Protokol + USB | Bước 7, 9 | Cần soạn |
| Phiếu thông báo kích thước thùng đề-bài thi | Bước 9 | Cần soạn |

## Khi có mẫu biểu mới

Báo cho người quản trị Handbook để thêm vào đúng vị trí, không tự thêm file rời rạc.

## Lịch sử thay đổi

- 2026-07-24: Biên soạn lại theo văn phong nội bộ PNE.
